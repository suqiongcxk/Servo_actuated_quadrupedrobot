...
//		printf("%02X\n",ID2);
    ...
    if(FRAME_type == DATA_Frame)    //这是一个数据报文，让我看看是什么数据
    {
        ...
        if(crc_Frme == crc_cal )  //CRC校验通过
        {
            ...
            {
							case Slave1_ID:
               target_angle_leftleg[0]  = float_ARRY[0]; //髋
               target_angle_leftleg[1]  = float_ARRY[1]; //小腿
							 target_angle_leftleg[2]  = float_ARRY[2]; //大腿
							 target_angle_RIGHTleg[0] = float_ARRY[3]; //髋
               target_angle_RIGHTleg[1] = float_ARRY[4]; //小腿
							 target_angle_RIGHTleg[2] = float_ARRY[5]; //大腿
            }
