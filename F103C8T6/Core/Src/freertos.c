...
#define  SCurve_turn     1
#define  Direct_turn     2
int servo_start  = 0;
int turn_mode   = SCurve_turn;
...
//      printf("%.2f\n",Left_front_thigh_angle);
    ...
    //右髋				
		Servo_SCurve_Control(Right_hip_channel,
												...
												RIGHT_HIP_ID+1); 
		}else if (turn_mode == Direct_turn )
		{
		
		
		Servo_Direct_Control(Left_thigh_channel,
										MOVE_TARget[LEFT_THIGH_ID],
										&CURRENT_ANGLE[LEFT_THIGH_ID]);
												
		Servo_Direct_Control(Left_lower_leg_channel,
                        MOVE_TARget[LEFT_LOWER_LEG_ID],
                        &CURRENT_ANGLE[LEFT_LOWER_LEG_ID]);	

		Servo_Direct_Control(Left_hip_channel,
                        MOVE_TARget[LEFT_HIP_ID],
                        &CURRENT_ANGLE[LEFT_HIP_ID]);
												
		Servo_Direct_Control(Right_thigh_channel,
                        MOVE_TARget[RIGHT_THIGH_ID],
                        &CURRENT_ANGLE[RIGHT_THIGH_ID]);	

		Servo_Direct_Control(Right_lower_leg_channel,
                        MOVE_TARget[RIGHT_LOWER_LEG_ID],
                        &CURRENT_ANGLE[RIGHT_LOWER_LEG_ID]);
												
		Servo_Direct_Control(Right_hip_channel,
												MOVE_TARget[RIGHT_HIP_ID],
												&CURRENT_ANGLE[RIGHT_HIP_ID]);		
		}												
		osDelay(1);
...
//			if ( __fabs( MOVE_TARget[LEFT_THIGH_ID] - CURRENT_ANGLE[LEFT_THIGH_ID]   ) < 0.5 ){ turn_mode = Direct_turn; }
			osDelay(4);
    }
