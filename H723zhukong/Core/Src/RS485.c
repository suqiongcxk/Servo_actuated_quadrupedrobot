...
float target_angle_1[6] = {0};
float target_angle_2[6] = {0};
float target_angle_3[6] = {0};
float target_angle_4[6] = {0};
//应答数据定义
int  Slave1_ACK   = 0;
int  Slave2_ACK   = 0;
int  Slave3_ACK   = 0;
...
void  RS485_SendBytes(  uint16_t *buf, uint16_t len_bytes, int  count_SET ) 
{
    ...
    RS485_TX_EN;
	  delay_us(10);
    /* Callers pass sizeof(buf): the length is already in bytes. */
    HAL_UART_Transmit_DMA(&huart1, (uint8_t *) buf, len_bytes);
...
//		printf("%02X\n",ID2);
    ...
    if(FRAME_type == DATA_Frame)    //这是一个数据报文，让我看看是什么数据
    {
        ...
        if(crc_Frme == crc_cal )  //CRC校验通过
        {
            ...
            switch ( ID2 )
            {
                case Slave1_ID :target_angle_1[0] = float_ARRY[0]; break ;
                case Slave2_ID :target_angle_2[0] = float_ARRY[0]; break ;
                case Slave3_ID :target_angle_3[0] = float_ARRY[0]; break ;
                case Slave4_ID :target_angle_4[0] = float_ARRY[0]; break ;      
            }
...
{ 
	  start_cycle = DWT->CYCCNT;
		MASTER_SEND_Frame(Slave1_ID, target_angle_1,6 ,DATA_Frame); 		
		end_cycle = DWT->CYCCNT;
//		printf("%d ,",end_cycle -start_cycle );
		osDelay(2);	
	  start_cycle = DWT->CYCCNT;	
		MASTER_SEND_Frame(Slave2_ID, target_angle_2,6 ,DATA_Frame); 	
		end_cycle = DWT->CYCCNT;
